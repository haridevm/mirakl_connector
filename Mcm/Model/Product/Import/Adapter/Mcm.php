...
class Mcm extends AbstractAdapter
{
    ...
    public function processVariantProducts($parentProduct, $unlinkProductId = null)
    {
        ...
        foreach ($this->config->getStoresUsedForProductImport() as $store) {
            $this->productAction->updateAttributes($variantIds, [
    ...
    public function import(array $data)
    {
        ...
        // Update multi values attributes
        $separator = $this->config->getMultivalueAttributesSeparator();
        foreach ($product->getAttributes() as $attribute) {
            $attr = $attribute->getAttributeCode();
            if ($attribute->getFrontendInput() == 'multiselect' && is_string($product->getData($attr))) {
                $product->setData($attr, explode($separator, $product->getData($attr)));
            }
        }
