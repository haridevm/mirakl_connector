...
trait ProductTrait
{
    ...
    public function createConfigurableProduct(CategoryModel $category, array $data)
    {
        ...
        foreach ($this->mciHelper->getUniqueAttributes() as $attribute) {
        ...
        $parentProduct = $this->createProduct(
            ...
            $category,
            $data,
            ['use_config_manage_stock' => 1, 'is_in_stock' => 0]
        );

        // Force visibility on configurable
        $parentProduct->setVisibility($this->config->getDefaultVisibility());
