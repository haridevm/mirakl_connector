...
class ValueList extends AbstractHelper implements ExportInterface
{
    ...
    public function prepare(DataObject $attribute, $action = null)
    {
        ...
        if ($option = $attribute->getOption()) {
            ...
            $options = array_filter($options, function ($data) use ($option) {
                foreach ($option['value'] as $labels) {
                    if (($labels[0] ?? '') === $data['label']) {
                        return true;
                    }
                }
                return false;
            ...
            // Prepare options to delete
            $delete = array_keys($option['delete'] ?? [], '1', true);
            $deleteOptions = array_map(function ($value) use ($option) {
                return [
                    'value'  => $value,
            ...
            if (count($deleteOptions) === count($option['value'])) {
                // Only deleted options to handle
                $options = $deleteOptions;
            } else {
                $options = array_merge($options, $deleteOptions);
            }
        }
