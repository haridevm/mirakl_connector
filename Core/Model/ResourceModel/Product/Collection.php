...
class Collection extends \Magento\Catalog\Model\ResourceModel\Product\Collection
{
     ...
     * @return array
     */
    public function getParentProductsData(array $productIds, array $cols)
    {
        if (empty($productIds)) {
            return [];
        }

        $select = $this->getParentProductsSelect($productIds, $cols);

        $data = array_fill_keys($productIds, []);
    ...
    public function getParentProductIds(array $productIds)
    {
        ...
        $select = $this->getParentProductsSelect($productIds, ['parent_id' => 'entity_id']);
    ...
    public function getParentProductsSelect(array $productIds, array $cols)
    {
        ...
        $select = $this->_conn->select()
            ->from(['cpsl' => $this->getTable('catalog_product_super_link')], ['product_id'])
            ->join(
                ['cpe' => $this->getTable('catalog_product_entity')],
                "cpsl.parent_id = cpe.$entityCol",
                $cols
            )
    ...
    public function getProductImages(array $productIds)
    {
        ...
        while ($row = $stmt->fetch()) {
            ...
            // Make sure no image is duplicated
            $file = !empty($row['file']) ? $row['file'] : $row['file_default'];
            if (isset($files[$productId]) && in_array($file, $files[$productId])) {
                continue;
            }
    ...
    {
        if ($this->getFlag('parent_data_overridden') || empty($this->_items)) {
            return $this;
        }
        ...
        foreach ($collection as $data) {
            ...
            foreach ($entityIds as $entityId) {
                // If product has multiple parents, keep data from the first one
                ...
                foreach ($attribute as $alias => $code) {
                    ...
                    $this->_items[$entityId][$field] = $data[$code] ?? '';
                }
            }
        }

        $this->setFlag('parent_data_overridden', true);
