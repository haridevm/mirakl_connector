...
class Product extends ClientMCM implements SynchroResultInterface, ExportDataInterface
{
    ...
    public function import(array $data)
    {
        ...
        if (isset($data['updated_to'])) {
            $request->setUpdatedTo($data['updated_to']);
        }
    ...
    public function getDeleteProducts($deletedFrom = null, array $productIds = [])
    {
        ...
        if (!empty($deletedFrom)) {
            if (is_string($deletedFrom)) {
                $deletedFrom = new \DateTime($deletedFrom);
            }
            $request->setDeletedFrom($deletedFrom);
        }
        ...
        $this->_eventManager->dispatch('mirakl_api_get_delete_products_before', [
            'request' => $request
        ]);
