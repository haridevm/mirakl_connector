...
class Order extends ClientHelper\MMP
{
    ...
    public function getOrders(array $params, $paginate = false, $locale = null)
    {
        ...
        if ($paginate === false) {
            // The parameter paginate is no longer supported in Mirakl
            // For retro-compatibility, we query all orders with multiple paginated queries
            $orderItems = [];
            $request->setMax(self::MIRAKL_PAGINATION_MAX_VALUE);
            $offset = 0;

            do {
                $request->setOffset($offset);
                /** @var OrderCollection $response */
                $response = $this->send($request);
                $orderItems = array_merge($orderItems, $response->getItems());
                $offset += self::MIRAKL_PAGINATION_MAX_VALUE;
            } while ($response->count() && $response->count() >= self::MIRAKL_PAGINATION_MAX_VALUE);

            $orders = new OrderCollection($orderItems);
        } else {
            $orders = $this->send($request);
        }
