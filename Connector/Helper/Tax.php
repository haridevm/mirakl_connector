...
class Tax extends AbstractHelper
{
    ...
    {
        $mirakOrderCalculatedTaxes = $this->getMiraklCalculatedTaxes($order);
        $cancelationAndRejectionTaxes = $this->getMiraklCancelationAndRejectionTaxes($order, $miraklOrder);
        foreach ($mirakOrderCalculatedTaxes as &$tax) {
            $tax['final_tax_amount']      = $tax['tax_amount'];
            $tax['final_base_tax_amount'] = $tax['base_tax_amount'];
            foreach ($cancelationAndRejectionTaxes as $taxCollection) {
                foreach ($taxCollection as $canceledTax) {
                    if ($tax['type'] === $canceledTax->getCode()) {
                        $tax['final_tax_amount']      -= (float) $canceledTax->getAmount();
                        $tax['final_base_tax_amount'] -= (float) $canceledTax->getBaseAmount();
                    }
                }
            }
        }

        return $mirakOrderCalculatedTaxes;
    }
            ...
            foreach ($order->getAllItems() as $item) {
                ...
                if ($miraklShippingTaxApplied = unserialize($item->getMiraklShippingTaxApplied())) { // phpcs:ignore
                    foreach ($miraklShippingTaxApplied as $miraklAppliedTax) {
                        ...
                        $fullTaxInfo[$miraklAppliedTax['id']]['is_shipping'] = true;
                    }
                }
            }
    ...
    {
        $mirakOrderCalculatedTaxes    = $this->getMiraklOrderCalculatedTaxes($order, $miraklOrder);
        $cancelationAndRejectionTaxes = $this->getMiraklCancelationAndRejectionTaxes($order, $miraklOrder);
        foreach ($mirakOrderCalculatedTaxes as &$tax) {
            $tax['final_tax_amount']      = $tax['tax_amount'];
            $tax['final_base_tax_amount'] = $tax['base_tax_amount'];
            foreach ($cancelationAndRejectionTaxes as $taxCollection) {
                foreach ($taxCollection as $canceledTax) {
                    if ($canceledTax->getData('is_shipping')) {
                        if (isset($tax['is_shipping']) && $tax['is_shipping']) {
                            $tax['final_tax_amount']      -= (float) $canceledTax->getAmount();
                            $tax['final_base_tax_amount'] -= (float) $canceledTax->getBaseAmount();
                        }
                    } elseif ($tax['id'] === $canceledTax->getCode()) {
                        $tax['final_tax_amount']      -= (float) $canceledTax->getAmount();
                        $tax['final_base_tax_amount'] -= (float) $canceledTax->getBaseAmount();
                    }
                }
    ...
    public function getMiraklCancelationAndRejectionTaxes(SalesOrder $order, MiraklOrder $miraklOrder)
    {
        ...
        return $cancelationAndRejectionTaxes;
    }
    ...
    protected function getMiraklOrderTaxItemsByOfferId($orderId, $offerId)
    {
        ...
        $select = $connection->select()
            ...
            ->join(
                ...
                ['row_total', 'base_row_total', 'mirakl_shipping_excl_tax', 'mirakl_base_shipping_excl_tax']
            )
