...
class TriggerOrderDebitObserver implements ObserverInterface
{
    /**
     * @var OrderFactory
     */
    ...
    /**
     * Method used by PSP to trigger order debit on an order by just dispatching an event
     * called 'mirakl_trigger_order_debit' with the following parameters:
     *
     * increment_id             Magento order (required)
     * order_id                 @deprecated Use increment_id instead
     * mirakl_order_id          Mirakl order (required)
     * status                   Status of the payment (OK or REFUSED) (optional, OK if empty)
    ...
    public function execute(Observer $observer)
    {
        ...
        $this->paymentApi->debitPayment(
            $miraklOrder,
            $miraklOrder->getCustomer()->getId(),
            $status,
